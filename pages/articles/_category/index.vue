<template>
	<div class="my-grid">
		...
		<div class="md:col-span-3 space-y-10">
			<nuxt-content class="static font-sans text-lightblue underline-links leading-relaxed" :document="category" />
	...
	export default {
		async asyncData({$content, params, error}) {
			try {
				...
				const posts = await $content("posts").where({category : category.titre}).only(["titre", "couverture", "url", "slug", "auteur", "category", "date"]).sortBy("date", "desc").fetch();
				posts.forEach((p, i) => {
