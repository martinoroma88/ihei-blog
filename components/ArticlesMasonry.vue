<template>
  <section class="grid grid-cols-1 md:grid-cols-6 gap-6 md:gap-3 lg:gap-5">
    <div
      v-for="(p, i) in posts"
      :key="p.slug || i"
    ...
    >
      <div class="relative rounded overflow-hidden" :class="mediaHeightClass(i)">
        ...
        <template v-else>
          <!-- Nessuna immagine: su desktop usa sfondo azure (#00B5E2) e testo bianco -->
          <div class="hidden md:block absolute inset-0 pointer-events-none bg-azure"></div>
          <div class="hidden md:block absolute inset-x-0 bottom-0 p-4 text-white">
            <p v-if="p.url">
              <a :class="['link font-bold font-sans text-white', i === 0 ? 'text-xl md:text-2xl' : '']" :href="p.url" target="_blank" :title="p.titre">{{ truncate(p.titre, 66) }}</a>
            </p>
      ...
      <!-- Testo sotto l'immagine: solo mobile -->
