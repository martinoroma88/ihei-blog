<template>
	...
	<nav class="w-full flex items-center justify-between lg:space-x-8 mb-4">
		...
		<div @click="toggle()" class="lg:hidden w-6 h-10 py-2 flex flex-col justify-around cursor-default relative z-10">
			<div :class="open ? 'translate-y-2' : 'translate-y-0'" class="transform transition duration-100 w-full h-1 bg-blue rounded-sm"></div>
			<div :class="open ? 'scale-x-0' : 'scale-x-1'" class="transform origin-right transition duration-75 w-full h-1 bg-blue rounded-sm"></div>
			<div :class="open ? 'scale-x-0' : 'scale-x-1'" class="transform transition duration-100 w-full h-1 bg-blue rounded-sm"></div>
		</div>
		...
		<div ref="items" :class="open ? 'scale-100' : 'scale-0 lg:scale-100'" class="transform absolute  lg:static inset-0 flex items-center justify-center lg:justify-start flex-col lg:flex-row space-y-6 lg:space-y-0 lg:space-x-6 h-screen lg:h-auto w-full lg:w-2/4">
			...
			<a :class="open ? 'translate-y-0 opacity-100 delay-200' : 'translate-y-12 lg:translate-y-0 opacity-0 lg:opacity-100'" class="lg:hidden transform transition duration-150 relative hover:bg-black hover:text-white no-underline border-black border-2 rounded px-2" target="_blank" rel="nofollow" href="https://www.youtube.com/channel/UC5pChIcpowCGXCTUg1exCHg/?guided_help_flow=5">IHEI TV</a>
		</div>
