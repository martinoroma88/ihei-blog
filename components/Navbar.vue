<template>
	<!-- NAVBAR -->
	<nav :class="accueil ? 'accueil' : ''" class="w-full grid grid-cols-4 gap-16 items-center justify-between border-b-2 border-gray-200 mb-8 pb-4 z-30 font-sans realtive">
		<!-- BRAND -->
		<div class="z-10 relative col-span-3 lg:col-span-1">
			<div>
		...
		<div :class="open ? 'opacity-100 delay-75 h-full' : 'delay-400 opacity-0'" class="lg:hidden transition duration-200 bg-blue inset-0 fixed z-0"></div>
		...
		<!-- NAVIGATION ITEMS -->
		<div ref="items" :class="open ? 'scale-100' : 'scale-0 lg:scale-100'" class="transform absolute lg:static inset-0 flex items-center justify-center lg:justify-between flex-col lg:flex-row space-y-6 lg:space-y-0 h-screen lg:h-auto w-full col-span-3" >
			<!-- Internal links -->
			...
			<div @click="toggle()" :class="open ? 'translate-y-0 opacity-100 delay-300' : 'translate-y-12 lg:translate-y-0 opacity-0 lg:opacity-100'" class="transform transition duration-150 mb-6 lg:mb-0 text-white lg:text-blue lg:hidden">
				<IconUp />
			</div>
		</div>
	</nav>
